...
// OPENAP_JSON_CONVERT_FUNCS(ContainerUpdateResponse, Warnings)

void ContainerUpdateResponse::ToJSON(Json & j) const
{
    // OPENAPI_FOR_EACH(OPENAPI_TO_JSON_MEMBER, __VA_ARGS__)
    j.AddMember<decltype(this->warnings)>(openapi::StringT(OPENAPI_LITERAL(Warnings)), this->warnings);
}

void ContainerUpdateResponse::FromJSON(const Json & j)
{
