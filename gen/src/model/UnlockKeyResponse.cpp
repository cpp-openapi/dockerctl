...
// OPENAP_JSON_CONVERT_FUNCS(UnlockKeyResponse, UnlockKey)

void UnlockKeyResponse::ToJSON(Json & j) const
{
    // OPENAPI_FOR_EACH(OPENAPI_TO_JSON_MEMBER, __VA_ARGS__)
    j.AddMember<decltype(this->unlock_key)>(openapi::StringT(OPENAPI_LITERAL(UnlockKey)), this->unlock_key);
}

void UnlockKeyResponse::FromJSON(const Json & j)
{
